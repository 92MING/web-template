...
class NestedLoopPolicy(asyncio.DefaultEventLoopPolicy):
    ...
    def get_child_watcher(self):
        if self._watcher is None:
            # PidfdChildWatcher is the best practice for Linux 3.9+ 
            # to handle subprocesses in multi-threaded environments.
            if sys.platform != "win32":
                self._watcher = asyncio.PidfdChildWatcher()
            else:
                # Windows doesn't use child watchers for subprocesses
                raise NotImplementedError("Child watchers not used on Windows")
