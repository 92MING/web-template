...
def run_in_background(func:Callable, args:tuple|None=None, kwargs: dict|None=None, timeout: int|None=120):  # type: ignore
    ...
    if is_async_callable(func):
        def wrapper(f, ctx: contextvars.Context, *args, **kwargs):
            def _inner():
        ...
        return _pool.submit(wrapper, func, ctx, *args, **kwargs)
    else:
        def wrapper(f, ctx: contextvars.Context, *args, **kwargs):
            def _inner():
                try:
        ...
        ctx = contextvars.copy_context()
        f = _pool.submit(wrapper, func, ctx, *args, **kwargs)
        if timeout and timeout > 0: # type: ignore
...
def is_async_callable(func:Callable)->TypeGuard[Callable[..., Awaitable]]:
    ...
    if hasattr(func, '__is_async_func__'):
        def func_arg_count(func, ignore_cls: bool=False):
            origin_func = func
...
def _on_program_exit():
    ...
    if async_jobs:
        ...
        try:
            ...
            loop.run_until_complete(_run_async_jobs(async_jobs))
            loop.close()
        except Exception as e:
            _logger.error(f'Error running async exit jobs: {e}')
