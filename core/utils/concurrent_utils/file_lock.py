...
class _FileCrossProcessLockBase(ABC):
    ...
    __slots__ = ('lock_id', 'lock_dir', 'lock_file', 'fd', 'default_timeout')
    ...
    def __init__(
        ...
        if not lock_dir:
            _instance = os.environ.get('__SERVER_INSTANCE_ID__')
            if _instance:
                self.lock_dir = os.path.join(_TEMP_DIR, _DEFAULT_LOCK_DIR, _instance)
            else:
    ...
    def wait(self, timeout: float|None=_empty)->bool:   # type: ignore
        if timeout is _empty:
...
class FileCrossProcessLock(_FileCrossProcessLockBase):
    ...
    def acquire(
        self, 
        blocking: bool=True, 
        timeout: float|None=_empty  # type: ignore
    )->bool:
        '''
        Acquire the lock.
        Args:
            ...
            timeout (float|None): Maximum time to wait for the lock in seconds. 
    ...
    @no_type_check
    def _acquire_unix(self, blocking: bool=True, timeout: float|None=_empty)->bool: # type: ignore
        if timeout is _empty:
            timeout = self.default_timeout
        # Clean up stale lock file from dead processes before acquiring
        self._try_remove_stale_lock_unix()
    ...
    def _acquire_windows(self, blocking: bool=True, timeout: float|None=_empty)->bool:
        if timeout is _empty:
            timeout = self.default_timeout
        start_time = time.time()
        t = _BACKOFF
        while True:
            try:
                ...
                # Try to remove stale lock left by a dead process.
                # This is safe inside the busy-wait loop because it
                # never opens the file (it only probes with unlink).
                if self._try_remove_stale_lock_win():
                    t = _BACKOFF
                    continue
    ...
    def _try_remove_stale_lock_win(self) -> bool:
        '''Remove a stale lock file left by a dead process on Windows.

        On Windows, ``open()`` does **not** set ``FILE_SHARE_DELETE``, so any
        open file handle prevents ``os.unlink()`` from succeeding.  We exploit
        this: if ``os.unlink()`` succeeds the file had *no* open handles,
        meaning the owning process already closed it (crashed or released but
        failed to delete).  If it fails with ``PermissionError`` the lock is
        still actively held — no need to read the PID at all.

        This avoids the race condition where opening the file for PID reading
        would itself block a concurrent ``release()`` from unlinking.
        '''
        try:
            os.unlink(self.lock_file)
            return True   # stale file removed
        except PermissionError:
            return False  # handle still open — actively held
    ...
    @no_type_check
    def _try_remove_stale_lock_unix(self) -> bool:
        '''Remove a stale lock file left by a dead process on Unix.

        On Unix, ``fcntl.flock`` is automatically released by the kernel when
        the holding process dies.  This method cleans up the leftover lock
        **file** for filesystem hygiene and to handle edge cases such as
        empty or corrupt PID files.
        ...
        try:
            ...
            return False
        try:
            # If anyone still holds the flock, this will raise
            fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except (IOError, BlockingIOError):
            fd.close()
            return False  # lock actively held
        # flock succeeded — no one holds the lock.  Check the PID.
        try:
    ...
    def release(self) -> bool:
        ...
        if os.name == 'nt':  # Windows
            self.fd.close()
            # Retry unlink: on Windows, os.unlink() fails while any other
            # process holds the file open (e.g. stale-lock reader).
            # A few retries with short delays cover that brief window.
