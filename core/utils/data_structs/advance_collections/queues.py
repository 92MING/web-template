from typing import Iterable, TYPE_CHECKING, Generator, overload, Self
...
def _find_order(lst: list[tuple], order: "Comparable", start: int=0, end: int=-1):
...
class AutoSortedQueue[OT: "Comparable", T]:
    '''
    ...
    elements: list[tuple[OT, T]]
    '''Elements in the queue, each element is a tuple of (priority, item)'''
    
    @overload
    def __init__(self):...
    @overload
    def __init__(self, objs: Iterable[T|OT]):...
    @overload
    def __init__(self, objs: Iterable[T], orders: Iterable[OT]):...
    
    def __init__(   # type: ignore
        self, 
        objs: Iterable[T]|None=None, 
        orders: Iterable[OT]|None=None
    ):
        self.elements = []
        if objs:
    ...
    def push(self, item: T, priority: OT|None=None)->None:
        '''
    ...
    def items(self)->Generator[tuple[OT, T], None, None]:
    ...
    def remove(self, item: T)->int|None:
    ...
    def find_order(self, order: OT)->tuple[int, T]|None:
        '''
    ...
    def remove_order(self, order: OT, max_remove: int|None=None)->int:
        '''
