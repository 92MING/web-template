...
_check_sub_cls_cache: dict[tuple[str, str], bool] = {}  # type: ignore
...
def _get_type_from_str(t: str):
    ...
    try:
        ...
        if real_t := locate(t):
            return real_t
        global_type_dict = _typing_global_dict().copy()
...
def _direct_check_sub_cls(sub_cls: type | str, super_cls: type | str):
    if __name__ == "__main__":
        from app.core.utils.type_utils.type_helpers import get_cls_name, get_sub_clses, get_origin, get_args 
    else:
...
def check_type_is(sub_cls, super_cls):
    ...
    """
    if __name__ == "__main__":
        from app.core.utils.type_utils.type_helpers import get_cls_name, get_sub_clses
    else:
