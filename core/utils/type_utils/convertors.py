...
from typing import (Any, Sequence, TypeAlias, overload, no_type_check, Iterable, 
                    Literal, TYPE_CHECKING, TypeAliasType, get_origin as tp_get_origin,
                    Annotated, Union)
from typing import (
...
__serializable_types__: dict[str, bool] = {}    # type: ignore
...
__pydantic_type_adapters__ = {}
"""{type or type_name, adapter}"""
...
def get_pydantic_type_adapter(t) -> TypeAdapter:
    ...
    try:
        ...
        if _save_isinstance(t, TypeAliasType):
            cache_key = f'{get_module_name(t)}{t.__name__}'
        else:
            cache_key = t if isinstance(t, str) else get_cls_name(t, with_module_name=True)
    
    if cache_key in __pydantic_type_adapters__:
        return __pydantic_type_adapters__[cache_key]
    adapter = TypeAdapter(t)
    __pydantic_type_adapters__[cache_key] = adapter
    return adapter
    ...
    try:
        ...
        if target_type:
            if _save_issubclass(target_type, datetime):
                ...
                from ..text_utils import get_num_from_text
                if (val.startswith('"') and val.endswith('"')) or (val.startswith("'") and val.endswith("'")):
                    val = val[1:-1]
                maybe_val = get_num_from_text(val, raise_error=False)
