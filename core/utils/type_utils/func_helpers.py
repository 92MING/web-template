...
_empty_method_cache: dict[str, bool] = {}   # type: ignore

def _is_empty_method_bytecode(code) -> bool | None:
    """
    Fast bytecode-based check for empty methods. Returns True/False if
    deterministic, or None when bytecode analysis is inconclusive (caller
    should fall back to source inspection).
    
    Recognised patterns (Python 3.10 – 3.13+):
      • pass / docstring-only / ``...`` / ``return`` / ``return None``
        → RESUME + RETURN_CONST(None) — typically 4 bytes
      • ``raise NotImplementedError``
        → RESUME + LOAD_GLOBAL(NotImplementedError) + RAISE_VARARGS(1)
      • ``raise NotImplementedError()``
        → RESUME + LOAD_GLOBAL(NotImplementedError+NULL) + CALL(0) + RAISE_VARARGS(1)
    """
    # Fast reject: empty methods have very short bytecode.
    # pass/docstring/return = 4 bytes, raise NotImplementedError() = 22 bytes.
    # Anything significantly longer is definitely not empty.
    if len(code.co_code) > 30:
        return False
    
    import dis
    # Gather meaningful instructions (skip housekeeping ops)
    _SKIP_OPS = frozenset((
        'RESUME', 'NOP', 'COPY_FREE_VARS', 'PRECALL',
        'PUSH_NULL', 'POP_TOP',
    ))
    meaningful = []
    for ins in dis.get_instructions(code):
        if ins.opname in _SKIP_OPS:
            continue
        # RETURN_CONST / RETURN_VALUE returning None
        if ins.opname == 'RETURN_CONST':
            continue  # RETURN_CONST always ends the func; skip it
        if ins.opname == 'RETURN_VALUE':
            continue
        # LOAD_CONST of None / Ellipsis / docstring → not real logic
        if ins.opname == 'LOAD_CONST':
            v = ins.argval
            if v is None or v is ... or isinstance(v, str):
                continue
        meaningful.append(ins)

    if not meaningful:
        return True  # pass / docstring / ... / return / return None

    # Check for `raise NotImplementedError` or `raise NotImplementedError()`
    ops = [(i.opname, i.argval) for i in meaningful]
    has_nie = any(v == 'NotImplementedError' for _, v in ops)
    has_raise = any(o == 'RAISE_VARARGS' for o, _ in ops)
    if has_nie and has_raise:
        # Only allow LOAD_GLOBAL, CALL variants, and RAISE_VARARGS
        allowed = {'LOAD_GLOBAL', 'RAISE_VARARGS', 'CALL', 'CALL_FUNCTION',
                    'CALL_FUNCTION_EX', 'CALL_INTRINSIC_1'}
        if all(o in allowed for o, _ in ops):
            return True

    return False  # has real logic
...
def is_empty_method(method: Callable):
    ...
    Uses fast bytecode inspection; falls back to source analysis when
    bytecode is inconclusive (e.g. builtins without ``__code__``).
    '''
    ...
    # ── fast path: bytecode inspection (no file I/O) ──
    func = method
    ...
    if code is not None:
        result = _is_empty_method_bytecode(code)
        if result is not None:
            _empty_method_cache[method_name] = result
            return result

    # ── slow fallback: source inspection ──
    is_empty = False
