...
from typing import (
    Annotated, Any, Awaitable, ClassVar, TypedDict, Sequence, Mapping, NotRequired, Required, Literal, Callable, TYPE_CHECKING,
...
class ServiceClientInitParams(TypedDict, total=False):
    ...
    '''外部注入的 aiohttp session。给定时客户端只复用它，不负责关闭。'''
...
class OpenAILikedClientCreateParams(ServiceClientInitParams, total=False):
    '''OpenAI 协议兼容客户端通用创建参数。'''
    apikey: str | None
    '''显式指定的 API Key。'''
    base_url: str | None
    '''显式指定的兼容接口根地址。'''
    ...
    '''SSH tunnel 配置；给定时会将兼容接口根地址改写到本地转发端口。'''

class OpenRouterClientCreateParams(OpenAILikedClientCreateParams, total=False):
    '''OpenRouter 协议兼容客户端通用创建参数。'''
    pass
...
class ServiceInitParams(TypedDict, total=False):
    ...
    fail_cooldown: float
    '''客户端失败后的基础冷却秒数。'''
    recovery_interval: 'ProbeInterval | float | None'
    '''后台健康恢复探测间隔；`None` 表示关闭恢复线程。'''
    init_probe: bool
    '''服务初始化后是否立即启动后台健康探测。'''


class ProbeInterval(BaseModel):
    '''Probe 调度策略。

    ``interval`` 是基础探测间隔；``decay`` > 1 时，会按客户端空闲时长逐步放大间隔；
    ``max_interval`` 用于限制放大后的上限。
    '''
    ...
    @classmethod
    def FromValue(cls, value: 'ProbeInterval | float | int | Mapping[str, object] | None') -> 'ProbeInterval | None':
        if value is None:
            return None
        if isinstance(value, ProbeInterval):
            return value.model_copy(deep=True)
        if isinstance(value, Mapping):
            return cls.model_validate(dict(value))
        return cls(interval=float(value))

    def normalized_interval(self) -> float:
        return max(1.0, float(self.interval))

    def normalized_decay(self) -> float:
        return max(1.0, float(self.decay))

    def normalized_max_interval(self) -> float | None:
        if self.max_interval is None:
            return None
        return max(self.normalized_interval(), float(self.max_interval))
    ...
    def effective_interval(
        ...
        *,
        ...
        base_interval = self.normalized_interval()
        decay = self.normalized_decay()
        max_interval = self.normalized_max_interval()
        if decay <= 1.0:
            return max_interval if max_interval is not None and max_interval < base_interval else base_interval
        current_time = time.time() if now is None else float(now)
        anchor = float(last_called_at or 0.0)
        if anchor <= 0:
            anchor = float(created_at or 0.0)
        if anchor <= 0:
            return max_interval if max_interval is not None and max_interval < base_interval else base_interval
        idle_seconds = max(0.0, current_time - anchor)
        steps = int(idle_seconds // base_interval)
        if steps <= 0:
            return base_interval
        next_interval = base_interval * (decay ** steps)
        if max_interval is not None:
            return min(max_interval, next_interval)
        return max(base_interval, next_interval)
...
class _RuntimeManagedClient(Protocol):
    ...
    _state_last_called_at: float
    _state_created_at: float
...
def _normalize_client_registry_key(
    service_kind: AIServiceKind | str | type | None,
    type_name: str,
) -> tuple[str, str]:
    '''将 service kind 与 type 规范化为注册表 key。'''
    return _normalize_service_kind_name(service_kind), _normalize_client_type(type_name)
...
class ServiceClientBase(ServiceCallLogMixin, ABC, Generic[ProbeInputT]):
        ...
        **kwargs: Any,
        ...
        if type is not None:
            ...
            if registry_key in _CLIENT_TYPE_REGISTRY:
                existing = _CLIENT_TYPE_REGISTRY[registry_key]
                raise TypeError(
                    f"Client type {type!r} under service kind {registry_key[0]!r} "
        ...
        if own_init is not None:
            ...
            def _guarded_init(self: 'ServiceClientBase', *args: Any, **kw: Any) -> None:
                ...
                try:
                    ...
                    if cache_key is not None:
                        self.__class__._instance_cache.pop(cache_key, None)
                    self._closed = True
                    self._close_reason = 'initialization failed'
                    raise
    ...
    def __init__(self, **kwargs: Unpack[ServiceClientInitParams]):
        ...
        self._state_last_called_at: float = 0.0
        self._state_created_at: float = time.time()
        injected_session = kwargs.get('aiohttp_session')
        if injected_session is not None and not isinstance(injected_session, aiohttp.ClientSession):
            raise TypeError(
                f'Invalid aiohttp_session argument; expected aiohttp.ClientSession but got {type(injected_session).__name__}'
            )
        if injected_session is not None and injected_session.closed:
            raise RuntimeError(f'{type(self).__name__} received a closed aiohttp_session.')
        self._cached_session: aiohttp.ClientSession | None = injected_session
        self._cached_session_owned: bool = injected_session is None
        self._cached_session_limit: int | None = None
        self._cached_session_loop: asyncio.AbstractEventLoop | None = self._get_session_loop(injected_session)
    ...
    @staticmethod
    def _get_session_loop(session: aiohttp.ClientSession | None) -> asyncio.AbstractEventLoop | None:
        loop = getattr(session, '_loop', None)
        return loop if isinstance(loop, asyncio.AbstractEventLoop) else None

    def _create_managed_session(self, *, limit: int) -> aiohttp.ClientSession:
        connector = aiohttp.TCPConnector(limit=limit, limit_per_host=limit)
        return aiohttp_client_session(connector=connector)

    async def _close_owned_session_async(
        self,
        ...
        *,
        ...
        if session_loop is current_loop:
            await session.close()
            return
        if session_loop is not None and not session_loop.is_closed() and session_loop.is_running():
            try:
                close_future = asyncio.run_coroutine_threadsafe(session.close(), session_loop)
    ...
    def _close_owned_session_sync(
        ...
        *,
        ...
        try:
            ...
            current_loop = None
        if session_loop is not None and session_loop is not current_loop and not session_loop.is_closed() and session_loop.is_running():
            try:
                close_future = asyncio.run_coroutine_threadsafe(session.close(), session_loop)
                close_future.result(timeout=1.0)
                return
    ...
    async def _get_session(self) -> 'aiohttp.ClientSession':
        ...
        owns_session = bool(getattr(self, '_cached_session_owned', True))
        prev_limit = getattr(self, '_cached_session_limit', None)
        prev_loop = getattr(self, '_cached_session_loop', None)
        if s is not None and not s.closed:
            if not owns_session:
                if prev_loop is not None and prev_loop is not current_loop:
                    raise RuntimeError(
                        f'{type(self).__name__} received an injected aiohttp_session bound to a different event loop.'
                    )
                return s
            if prev_limit == limit and prev_loop is current_loop:
                return s

        if s is not None and s.closed and not owns_session:
            raise RuntimeError(f'{type(self).__name__} injected aiohttp_session is already closed.')

        # limit 变化、session 已关闭或 event loop 不匹配 → 关掉旧的再重建
        if s is not None and not s.closed and owns_session:
            await self._close_owned_session_async(s, session_loop=prev_loop)

        s = self._create_managed_session(limit=limit)
        self._cached_session = s
        self._cached_session_owned = True
    ...
    def close(self, reason: str | None = None) -> None:
        ...
        self._cached_session_limit = None
        self._cached_session_loop = None
        if s is None or s.closed or not owns_session:
            return
        try:
            self._close_owned_session_sync(s, session_loop=prev_loop)
        except Exception:
            pass

    def _set_max_concurrent_value(self, value: object) -> None:
        if value is None:
            self.max_concurrent = None
            return
        if isinstance(value, ConcurrentPool):
            self.max_concurrent = value
            if self.max_concurrent.category is None:
                self.max_concurrent.category = self.key
            return
        next_value = max(1, int(cast(int | str, value)))
        pool = getattr(self, 'max_concurrent', None)
        if isinstance(pool, ConcurrentPool):
            pool.max_concurrent = next_value
            return
        self.max_concurrent = ConcurrentPool(getattr(self, 'key', None), next_value)

    def reset_health_state(self) -> None:
        '''重置与可用性判断相关的运行态。'''
        self.__dict__.pop('_cached_client_hash', None)
        self._state_score = 1.0
        self._state_success_count = 0
        self._state_fail_count = 0
        self._state_cooldown_until = 0.0
        self._state_last_error = None
        self._state_speed_ewma = 0.0
        self._state_last_success_at = 0.0
        self._state_last_called_at = 0.0
        if hasattr(self, '_state_last_probe_at'):
            object.__setattr__(self, '_state_last_probe_at', 0.0)

    def update(self, **new_params: object) -> bool:
        '''尝试原地更新客户端配置；无法安全更新时返回 False。'''
        if bool(getattr(self, '_closed', False)):
            return False
        raw_key = new_params.get('key')
        if raw_key is not None and str(raw_key) != str(getattr(self, '_key', '')):
            return False
        if 'max_concurrent' in new_params:
            self._set_max_concurrent_value(new_params['max_concurrent'])
        if 'priority' in new_params:
            priority_value = cast(Any, new_params['priority'])
            self.priority = float(priority_value or 0.0)
        if 'strategy_lvl' in new_params:
            strategy_value = cast(Any, new_params['strategy_lvl'])
            self.strategy_lvl = int(strategy_value or StrategyLevel.LOAD_BALANCE)

        for key, value in new_params.items():
            if key in {'key', 'max_concurrent', 'priority', 'strategy_lvl'}:
                continue
    ...
    def GetClientCls(
        type: str,
        *,
        service_kind: AIServiceKind | str | type | None = None,
    ) -> 'type[ServiceClientBase] | None':  # noqa: A002
        ...
        matches = [
            registered_cls
            for (registered_kind, registered_type), registered_cls in _CLIENT_TYPE_REGISTRY.items()
    ...
    @classmethod
    def RegisteredClientTypes(
        cls,
        *,
        service_kind: AIServiceKind | str | type | None = None,
    ) -> list[str]:
        '''返回已注册的客户端 type 列表。'''
        normalized_kind = _normalize_service_kind_name(service_kind)
        values = [
            registered.Type or registered.__name__
    ...
    def load_runtime_capabilities_from_shared_status(self, status: Mapping[str, object]) -> None:
        '''从跨进程共享状态恢复能力探测结果。'''
        return None
...
class ServiceBase(ServiceCallLogMixin, ABC, Generic[CT]):
    ...
    DefaultProbeIntervalSeconds: ClassVar[float] = 300.0
    '''Default main-process health probe interval when a service does not set recovery_interval.'''

    DefaultProbeInterval: ClassVar[ProbeInterval | float | None] = ProbeInterval(interval=300.0, decay=2.0, max_interval=86400.0)
    '''结构化 probe 调度默认值；允许子类覆盖。'''
    ...
    def __init__(self, *clients: CT | ServiceClient[CT], **kwargs: Unpack[ServiceInitParams]):
        ...
        recovery_interval = kwargs['recovery_interval'] if 'recovery_interval' in kwargs else _empty
        self._fail_cooldown = max(1.0, fail_cooldown)
        self._recovery_interval: ProbeInterval | None = self._normalize_probe_interval_value(recovery_interval)
        ...
        self._init_probe_enabled = bool(kwargs.get('init_probe', False))
        ...
        if self._init_probe_enabled and not (os.getenv('IN_FASTAPI_WORKER') == '1' or os.getenv('__SERVER_SUPERVISOR_PID__')):
            self._start_init_probe()

    @classmethod
    def _normalize_probe_interval_value(cls, value: object) -> ProbeInterval | None:
        if value is _empty:
            return ProbeInterval.FromValue(cls.DefaultProbeInterval)
        return ProbeInterval.FromValue(cast(ProbeInterval | float | int | Mapping[str, object] | None, value))

    def _probe_interval_policy(self) -> ProbeInterval | None:
        raw = getattr(self, '_recovery_interval', None)
        return self._normalize_probe_interval_value(raw)

    def _effective_probe_interval(self, client: ServiceClientBase | None = None, *, now: float | None = None) -> float | None:
        policy = self._probe_interval_policy()
        if policy is None:
            return None
        if client is None:
            return policy.normalized_interval()
        runtime = cast(_RuntimeManagedClient, client)
        return policy.effective_interval(
            now=now,
            last_called_at=float(getattr(runtime, '_state_last_called_at', 0.0)),
            created_at=float(getattr(runtime, '_state_created_at', 0.0)),
        )
    ...
    def _resolve_service_client_candidates(
        self,
        clients: Sequence[CT | ServiceClient[CT]],
        client_key: str | None = None,
    ) -> list[ServiceClient[CT]]:
        target = str(client_key or '').strip()
        if not target:
            return self._service_client_sequence(clients)
        binding = self._find_service_client(target)
        if binding is None:
            raise KeyError(f'Client {target!r} is not attached to {type(self).__name__}.')
        return [binding]
    ...
    def update(
        self,
        *,
        clients: Sequence[CT | ServiceClient[CT]] | None = None,
        fail_cooldown: float | None = None,
        recovery_interval: ProbeInterval | float | None = _empty,
        **new_params: object,
    ) -> bool:
        '''尝试原地更新服务配置与绑定 client 列表。'''
        if bool(getattr(self, '_closed', False)):
            return False
        if clients is not None:
            self._clients = [self._normalize_service_client(client) for client in clients]
        if fail_cooldown is not None:
            self._fail_cooldown = max(1.0, float(fail_cooldown))
        if recovery_interval is not _empty:
            self._recovery_interval = self._normalize_probe_interval_value(recovery_interval)

        attr_aliases = {
            's2t_service': '_s2t_service',
            'completion_service': '_completion_service',
        }
        for key, value in new_params.items():
            if key in {'key', 'init_probe'}:
                continue
            attr_name = attr_aliases.get(key, key)
    ...
    def _should_skip_probe_from_shared_status(
        ...
        *,
        ...
        recovery_interval = float(self._effective_probe_interval(client, now=current_time) or self.DefaultProbeIntervalSeconds)
    ...
    def close(self, *, close_clients: bool = False) -> None:
        '''停止后台恢复线程并释放服务级运行资源。'''
        for instance_key, instance in list(ServiceBase.ServiceInstances.items()):
            if instance is self:
                ServiceBase.ServiceInstances.pop(instance_key, None)
        if bool(getattr(self, '_closed', False)):
            return
        self._closed = True
    ...
    def _ensure_recovery_task(self) -> None:
        if os.getenv('IN_FASTAPI_WORKER') == '1' or os.getenv('__SERVER_SUPERVISOR_PID__'):
            return
        if self._probe_interval_policy() is None:
            return
    ...
    async def _wait_recovery_interval(self) -> bool:
        interval = self._effective_probe_interval()
    ...
    async def _recovery_loop(self) -> None:
        assert self._probe_interval_policy() is not None
    ...
    async def _probe_client_runtime_capabilities_on_init(self, client: ServiceClientBase) -> None:
        probe = getattr(client, 'probe_runtime_capabilities_on_init', None)
        if not callable(probe):
            return
        try:
            await _async_run_any_func(cast(Callable[[], object], probe))
        except Exception:
            pass

    async def _probe_client_if_due(
        self,
        client: ServiceClientBase,
        *,
        ...
        current_time = time.time() if now is None else float(now)
        default_interval = self._effective_probe_interval(client, now=current_time)
        if interval is None:
            probe_interval = default_interval
        else:
            probe_interval = max(1.0, float(interval))
        if probe_interval is None:
            return None
        if bool(getattr(client, '_closed', False)):
            return None
        await _async_run_any_func(self._load_client_status_from_shared, client, max_age=probe_interval)
        last_probe = float(getattr(client, '_state_last_probe_at', 0.0))
        if last_probe > 0 and (current_time - last_probe) < probe_interval:
            return None
        last_success = float(getattr(client, '_state_last_success_at', 0.0))
        if last_success > 0 and (current_time - last_success) < probe_interval:
            return None
        if float(getattr(client, '_state_cooldown_until', 0.0)) > current_time:
            return None

        pre_fail_count = int(getattr(client, '_state_fail_count', 0))
        object.__setattr__(client, '_state_last_probe_at', time.time())
        healthy = await self._probe_client_min_health(client)
        runtime = cast(_RuntimeManagedClient, client)
        if int(getattr(runtime, '_state_fail_count', 0)) != pre_fail_count:
            return healthy
        if healthy:
            runtime._state_score = min(1.0, float(getattr(runtime, '_state_score', 1.0)) + 0.15)
            runtime._state_last_success_at = time.time()
            if int(getattr(runtime, '_state_fail_count', 0)) > 0:
                runtime._state_fail_count = max(0, int(getattr(runtime, '_state_fail_count', 0)) - 2)
            if int(getattr(runtime, '_state_fail_count', 0)) <= 0:
                runtime._state_cooldown_until = 0.0
            runtime._state_last_error = None
        else:
            fail_count = int(getattr(runtime, '_state_fail_count', 0)) + 1
            runtime._state_fail_count = fail_count
            runtime._state_score = max(0.05, float(getattr(runtime, '_state_score', 1.0)) * 0.7)
            runtime._state_cooldown_until = time.time() + min(self._fail_cooldown * (2 ** min(fail_count, 5)), 1800.0)
            runtime._state_last_error = 'probe_min_health returned false'
        if healthy and last_probe <= 0:
            await self._probe_client_runtime_capabilities_on_init(client)
        await _async_run_any_func(self._sync_client_status_to_shared, client)
        return healthy

    @classmethod
    async def ProbeRegisteredClientsOnce(cls, *, service_kinds: Sequence[str] | None = None) -> dict[str, int]:
        normalized_kinds = None if service_kinds is None else {str(kind).strip().lower() for kind in service_kinds}
        candidates: dict[str, tuple['ServiceBase', ServiceClientBase, float]] = {}
        for (service_cls, _key), service in list(ServiceBase.ServiceInstances.items()):
            if bool(getattr(service, '_closed', False)):
                continue
            service_kind = str(service_cls.ServiceKind()).strip().lower()
            if normalized_kinds is not None and service_kind not in normalized_kinds:
                continue
            for client in getattr(service, 'clients', []):
                if not isinstance(client, ServiceClientBase) or isinstance(client, ServiceBase):
                    continue
                interval = service._effective_probe_interval(client)
                if interval is None:
                    continue
                client_hash = compute_client_hash(client)
                existing = candidates.get(client_hash)
                if existing is None or interval < existing[2]:
                    candidates[client_hash] = (service, client, interval)
        if not candidates:
            return {'checked': 0, 'probed': 0, 'healthy': 0, 'unhealthy': 0}
        results = await asyncio.gather(*(
            service._probe_client_if_due(client, interval=interval)
            for service, client, interval in candidates.values()
    ...
    async def _sync_client_status_to_shared(self, client: ServiceClientBase) -> None:
        ...
        try:
            ...
            await _async_run_any_func(ctx.update_client_status, client_hash, {
                ...
                'last_called_at': float(getattr(runtime, '_state_last_called_at', 0.0)),
                **client.dump_runtime_capabilities_for_shared_status(),
            })
        except Exception:
            pass
    ...
    async def _load_client_status_from_shared(
        ...
        *,
        ...
        try:
            ...
            if 'last_called_at' in status:
                runtime._state_last_called_at = float(status['last_called_at'])
            client.load_runtime_capabilities_from_shared_status(status)
            return True
        except Exception:
            return False
    ...
    def _is_non_client_responsibility_error(self, exc: Exception) -> bool:
        if getattr(exc, 'ai_client_responsibility', None) is False:
            return True
        if isinstance(exc, aiohttp.ClientResponseError):
            return int(getattr(exc, 'status', 0) or 0) in {400, 413, 422}
        if isinstance(exc, (ValueError, TypeError)):
            return True
        exc_msg = str(exc).lower()
        request_error_fragments = (
            'context length', 'context window', 'maximum context', 'max context',
            'token limit', 'too many tokens', 'input is too long', 'prompt is too long',
            'invalid input', 'bad input', 'unsupported language', 'unsupported lang',
            'unsupported speaker', 'speaker not found', 'unknown speaker',
            'unsupported voice', 'voice not found', 'unknown voice',
        )
        return any(fragment in exc_msg for fragment in request_error_fragments)
...
def _rewrite_url_for_ssh_tunnel(url: str, ssh_tunnel: 'SSHTunnelConfig') -> str:
    ...
    if parsed.hostname and getattr(ssh_tunnel, 'remote_ip', None) == '127.0.0.1':
        ssh_tunnel.remote_ip = parsed.hostname
...
class OpenAILikedClientMixin:
    '''OpenAI 协议兼容客户端通用 Mixin。'''
    ...
    @staticmethod
    def _normalize_openai_liked_extra_headers(extra_headers: Mapping[str, Any] | None) -> dict[str, str]:
        if not isinstance(extra_headers, Mapping):
            return {}
        normalized: dict[str, str] = {}
        for key, value in extra_headers.items():
            header_key = str(key or '').strip()
            if not header_key or value is None:
                continue
            normalized[header_key] = str(value)
        return normalized

    def _init_openai_liked_client(
        self,
        *,
        apikey: str | None,
        base_url: str,
        ...
        self._model = model
        self._extra_headers = self._normalize_openai_liked_extra_headers(extra_headers)

    def _set_openai_liked_extra_headers(self, extra_headers: Mapping[str, Any] | None) -> None:
        self._extra_headers = self._normalize_openai_liked_extra_headers(extra_headers)

    def _openai_liked_headers(self, content_type: str | None = 'application/json') -> dict[str, str]:
    ...
    def _openrouter_headers(self, content_type: str | None = 'application/json') -> dict[str, str]:
        headers = self._openai_liked_headers(content_type=content_type)
    ...
    def _openai_liked_endpoint(self, path: str) -> str:
        if self._base_url.endswith(path):
            return self._base_url
        return f'{self._base_url}{path}'

    def _openai_liked_endpoint_candidates(self, path: str) -> list[str]:
        primary = self._openai_liked_endpoint(path)
        candidates = [primary]
        parsed = urlparse(self._base_url)
        if (parsed.path or '/') == '/' and not path.startswith('/v1/'):
            fallback = f'{self._base_url}/v1{path}'
            if fallback not in candidates:
                candidates.append(fallback)
        return candidates


def _resolve_openai_liked_apikey(apikey: str | None, *, service_name: str = 'client', allow_empty: bool = False) -> str:
    key = apikey or _env_first('OPENAI_APIKEY', 'OPENAI_API_KEY')
    if not key:
        if allow_empty:
            return ''
        raise ValueError(f'OpenAI-liked {service_name} apikey is required.')
    return key


def _resolve_openai_liked_base_url(base_url: str | None, *, service_name: str = 'client') -> str:
    url = base_url or _env_first('OPENAI_API_URL', 'OPENAI_BASE_URL') or 'https://api.openai.com/v1'
    if not url:
        raise ValueError(f'OpenAI-liked {service_name} base_url is required.')
    return url


def _resolve_openrouter_apikey(apikey: str | None, *, service_name: str = 'client') -> str:
    key = apikey or _env_first('OPENROUTER_APIKEY', 'OPENROUTER_API_KEY')
    if not key:
        raise ValueError(f'OpenRouter {service_name} apikey not provided.')
    return key


def _resolve_openrouter_base_url(base_url: str | None) -> str:
    return base_url or _env_first('OPENROUTER_API_URL') or 'https://openrouter.ai/api/v1'


def _resolve_openai_liked_client_params(
    kwargs: Mapping[str, Any],
    *,
    service_name: str,
) -> dict[str, Any]:
    configured_base_url = cast(str | None, kwargs.get('base_url')) or _env_first('OPENAI_API_URL', 'OPENAI_BASE_URL')
    return {
        'apikey': _resolve_openai_liked_apikey(
            cast(str | None, kwargs.get('apikey')),
            service_name=service_name,
            allow_empty=bool(configured_base_url),
        ),
        'base_url': _resolve_openai_liked_base_url(configured_base_url, service_name=service_name),
        'model': cast(str | None, kwargs.get('model')),
        'ssh_tunnel': cast('SSHTunnelConfig | dict[str, Any] | str | None', kwargs.get('ssh_tunnel')),
    }


def _resolve_openrouter_client_params(
    kwargs: Mapping[str, Any],
    *,
    service_name: str,
    model_env_keys: Sequence[str] = (),
    default_model: str | None = None,
) -> dict[str, Any]:
    env_model = _env_first(*model_env_keys) if model_env_keys else None
    return {
        'apikey': _resolve_openrouter_apikey(cast(str | None, kwargs.get('apikey')), service_name=service_name),
        'base_url': _resolve_openrouter_base_url(cast(str | None, kwargs.get('base_url'))),
        'model': cast(str | None, kwargs.get('model')) or env_model or default_model,
        'ssh_tunnel': cast('SSHTunnelConfig | dict[str, Any] | str | None', kwargs.get('ssh_tunnel')),
    }


def _append_default_client(
    clients: list[Any],
    ...
    *,
    ...
    try:
        ...
        logger.warning('Failed to create %s default client: %s', description, exc)


def _append_env_default_client(
    clients: list[Any],
    factory: Callable[..., Any],
    *,
    logger: logging.Logger,
...
__all__ += [
    ...
    '_resolve_openai_liked_apikey',
    '_resolve_openai_liked_base_url',
    '_resolve_openrouter_apikey',
