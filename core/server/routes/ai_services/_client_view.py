...
def _get_service_class(kind: AIServiceKind) -> 'type[ServiceBase] | None':
    ...
    from core.ai import (
        ...
        CompletionService, EmbeddingService, S2TService, T2SService, T2ImgService,
...
class AIServiceClientInfo(AdvancedBaseModel):
    ...
    """ServiceClientBase.Type 厂商标识 (e.g. 'thinkthinksyn', 'openai', 'custom')。"""
...
def build_client_info(client: 'ServiceClientBase | ServiceClient[ServiceClientBase]') -> AIServiceClientInfo:
    ...
    return AIServiceClientInfo(
        ...
        last_probe_at=float(getattr(real_client, '_state_last_probe_at', 0.0) or 0.0),
...
def build_all_services_info() -> list[AIServiceInfo]:
    ...
    return [build_service_info(kind) for kind in ('completion', 'embedding', 's2t', 't2s', 't2img')]
