...
export class BuiltinWordCloud extends BuiltinBaseElement {
  ...
  static get styles() {
    return css`
      :host { display: block; min-height: var(--builtin-word-cloud-height, 420px); }
      .cloud-wrap {
        position: relative;
        width: 100%;
        height: 100%;
        min-height: var(--builtin-word-cloud-height, 420px);
      }
      canvas {
        display: block;
        width: 100%;
  ...
  _renderCloud() {
    ...
    const options = {
      ...
      clearCanvas: true,
      gridSize: Math.max(4, Math.floor(Math.min(w, h) / 58)),
      weightFactor: (size) => {
        const max = Math.max(...list.map((i) => (Array.isArray(i) ? i[1] : i.weight)));
        if (!max) return 18;
        return (size / max) * Math.min(w, h) * 0.26 + 14;
      },
      minSize: 12,
      rotateRatio: 0.2,
      ellipticity: 1,
