...
export class BuiltinFlowDesigner extends BuiltinBaseElement {
  ...
  async _init_graph() {
    ...
    this._wrap_canvas_resize();
    this._wrap_property_editor();
    this._apply_theme();
    this._seed_graph();
    this._graph.start();
  ...
  _wrap_canvas_resize() {
    if (!this._canvas || this._canvas.__builtin_resize_wrapped) return;
    const instance = this;
    const original = this._canvas.resize.bind(this._canvas);
    this._canvas.resize = function(w, h) {
      const shell = instance.renderRoot.querySelector(".canvas-shell");
      if (!shell) return original(w, h);
      const rect = shell.getBoundingClientRect();
      const cssWidth = w || Math.max(320, Math.round(rect.width));
      const cssHeight = h || Math.max(instance._ptMobile ? 360 : 320, Math.round(rect.height));
      const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 2));
      const canvas = instance._canvas.canvas;
      const bgcanvas = instance._canvas.bgcanvas;
      ...
      if (canvas.width !== targetWidth || canvas.height !== targetHeight) {
        ...
        instance._canvas.setDirty(true, true);
      }
    };
    this._canvas.__builtin_resize_wrapped = true;
  }

  _wrap_property_editor() {
    if (!this._canvas || this._canvas.__builtin_editor_wrapped) return;
    const self = this;
    const originalShowEditPropertyValue = this._canvas.showEditPropertyValue?.bind(this._canvas);
    this._canvas.showEditPropertyValue = function(node, property, options) {
      return self._show_inline_editor(node, property, options);
    };
    const originalPrompt = this._canvas.prompt?.bind(this._canvas);
    this._canvas.prompt = function(title, value, callback, event, multiline) {
      if (title === "Title" || title === "title") {
        const node = self._canvas.selected_nodes ? Object.values(self._canvas.selected_nodes)[0] : null;
        return self._show_inline_editor(node, "title", { event, value, callback });
      }
      return originalPrompt?.(title, value, callback, event, multiline);
    };
    this._canvas.__builtin_editor_wrapped = true;
  }

  _show_inline_editor(node, property, options) {
    const popup = document.createElement("div");
    popup.className = "builtin-flow-popup graphdialog builtin-flow-inline-editor";
    let labelText = property === "title" ? "Title" : (property.charAt(0).toUpperCase() + property.slice(1));
    let propType = "string";
    let propValues = null;
    if (node && property !== "title") {
      const info = node.getPropertyInfo?.(property) || {};
      propType = info.type || "string";
      if (info.label) labelText = info.label;
      propValues = info.values;
    }
    let inputHtml = "";
    if (property === "title" || propType === "string" || propType === "number" || propType === "array" || propType === "object") {
      inputHtml = `<input type="text" class="value" autofocus />`;
    } else if (propType === "boolean" || propType === "toggle") {
      inputHtml = `<input type="checkbox" class="value" />`;
    } else if ((propType === "enum" || propType === "combo") && propValues) {
      inputHtml = `<select class="value" autofocus>`;
      if (propValues.constructor === Array) {
        for (const v of propValues) {
          inputHtml += `<option value="${v}">${v}</option>`;
        }
      } else {
        for (const key in propValues) {
          inputHtml += `<option value="${propValues[key]}">${key}</option>`;
        }
      }
      inputHtml += `</select>`;
    } else {
    ...
    popup.innerHTML = `
      <div class="inline-editor-header">${labelText}</div>
      ${inputHtml}
    ...
    `;
    const input = popup.querySelector(".value");
    const initial = options?.value ?? (property === "title" ? node?.title : node?.properties?.[property]) ?? "";
    ...
    const save = () => {
      ...
      if (property === "title") {
        ...
        node.properties = { ...(node.properties || {}), [property]: val };
        if (property === "label") node.title = val;
        node.setDirtyCanvas(true, true);
        this._schedule_graph_sync();
      }
    ...
    popup.querySelector(".btn-confirm").addEventListener("click", () => { save(); close(); });
    popup.querySelector(".btn-cancel").addEventListener("click", () => { close(); });
    ...
    } else if (node && this._canvas) {
      const canvas = this._canvas.canvas;
      const rect = canvas.getBoundingClientRect();
      const nodeX = (node.pos[0] + this._canvas.ds.offset[0]) * this._canvas.ds.scale;
      const nodeY = (node.pos[1] + this._canvas.ds.offset[1]) * this._canvas.ds.scale;
      left = rect.left + nodeX;
      top = rect.top + nodeY;
    }

    popup.style.position = "fixed";
    popup.style.left = `${left + 8}px`;
    popup.style.top = `${top + 8}px`;

    document.body.appendChild(popup);
    this._prepare_popup(popup);
    if (input.tagName === "INPUT" && input.type === "checkbox") {
      input.focus();
    } else {
      input.focus();
  ...
  _ensure_popup_styles() {
    ...
    style.textContent = `
      ...
      .builtin-flow-inline-editor .inline-editor-header {
        font-size: 11px;
        font-weight: 700;
        letter-spacing: 0.08em;
        text-transform: uppercase;
        color: var(--builtin-flow-popup-muted, #9ca3af);
      }
      .builtin-flow-inline-editor input.value {
        width: 100% !important;
        min-height: 38px !important;
        margin: 0 !important;
        padding: 0 11px !important;
      ...
      .builtin-flow-inline-editor .inline-editor-actions {
        display: flex;
        justify-content: flex-end;
        gap: 8px;
      }
      .builtin-flow-inline-editor button {
      ...
      .builtin-flow-inline-editor .btn-cancel {
        background: transparent !important;
        color: var(--builtin-flow-popup-text, #111827) !important;
      }
      .builtin-flow-inline-editor .btn-confirm {
        background: var(--builtin-flow-popup-accent, #2563eb) !important;
        color: #ffffff !important;
        border-color: var(--builtin-flow-popup-accent, #2563eb) !important;
      }
      .builtin-flow-inline-editor input.value[type="checkbox"] {
        width: auto !important;
        min-height: 22px !important;
        height: 22px !important;
        margin: 4px 0 !important;
        accent-color: var(--builtin-flow-popup-accent, #2563eb);
      }
      .builtin-flow-inline-editor select.value {
        width: 100% !important;
        min-height: 38px !important;
        margin: 0 !important;
        padding: 0 11px !important;
  ...
  _apply_popup_theme(theme = this._theme_values()) {
    ...
    document.documentElement.style.setProperty("--builtin-flow-popup-muted", theme.muted);
  }
