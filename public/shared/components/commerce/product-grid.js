...
import { BuiltinBaseElement, html, css, styleMap, repeat } from "../lit-base.js";
...
export class BuiltinProductGrid extends BuiltinBaseElement {
  ...
  static styles = css`
    ...
    .thumb::before {
      ...
      background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--builtin-color-muted, #9ca3af) 14%, transparent), transparent);
      transform: translateX(-100%);
      animation: shimmer 1.2s linear infinite;
      opacity: 1;
    }
    .thumb.loaded::before, .thumb.empty::before { display: none; }
    .list .thumb { width: 140px; min-width: 140px; aspect-ratio: auto !important; }
    .thumb img { width: 100%; height: 100%; object-fit: cover; display: block; opacity: 0; transition: opacity .18s ease; }
    ...
    .rec-thumb { position: relative; aspect-ratio: 1; background: var(--builtin-header-bg, #f9fafb); overflow: hidden; }
    .rec-thumb::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--builtin-color-muted, #9ca3af) 14%, transparent), transparent); transform: translateX(-100%); animation: shimmer 1.2s linear infinite; }
    .rec-thumb.loaded::before, .rec-thumb.empty::before { display: none; }
    .rec-item img { width: 100%; height: 100%; object-fit: cover; display: block; opacity: 0; transition: opacity .18s ease; }
    .rec-item img.loaded { opacity: 1; }
  ...
  render() {
    ...
    return html`
      <div class="${mode} ${isList ? "list" : "grid"}" style="${styleMap(gridStyle)}">
        ${repeat(
          products,
          (p, index) => p.id || p.sku || p.image || this._title(p) || index,
          (p) => html`
            <div class="product" @click="${() => this._onClick(p.id)}">
              <div class="thumb ${p.image ? "" : "empty"}">
                ${p.image ? html`<img src="${p.image}" alt="${this._title(p)}" loading="lazy" @load=${this._markLoaded} @error=${this._markLoaded} />` : ""}
              </div>
              <div class="info">
                <div class="title">${this._title(p)}</div>
                <div class="price-row">
          ...
          <div class="rec-scroll">
            ${recs.map((r) => html`
              <div class="rec-item" @click="${() => this._onClick(r.id)}">
                <div class="rec-thumb ${r.image ? "" : "empty"}">${r.image ? html`<img src="${r.image}" alt="${this._title(r)}" loading="lazy" @load=${this._markLoaded} @error=${this._markLoaded} />` : ""}</div>
                <div class="t">${this._title(r)}</div>
